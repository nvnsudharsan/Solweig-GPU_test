...
def annulus_weight(altitude, aziinterval, device='cuda'):
...
def create_patches(patch_option):
    ...
    skyvaultalt = torch.tensor([], device='cuda')
    skyvaultazi = torch.tensor([], device='cuda')

    if patch_option == 1:
        annulino = torch.tensor([0, 12, 24, 36, 48, 60, 72, 84, 90], device='cuda')
        skyvaultaltint = torch.tensor([6, 18, 30, 42, 54, 66, 78, 90], device='cuda')
        azistart = torch.tensor([0, 4, 2, 5, 8, 0, 10, 0], device='cuda')
        patches_in_band = torch.tensor([30, 30, 24, 24, 18, 12, 6, 1], device='cuda')
    elif patch_option == 2:
        annulino = torch.tensor([0, 12, 24, 36, 48, 60, 72, 84, 90], device='cuda')
        skyvaultaltint = torch.tensor([6, 18, 30, 42, 54, 66, 78, 90], device='cuda')
        azistart = torch.tensor([0, 4, 2, 5, 8, 0, 10, 0], device='cuda')
        patches_in_band = torch.tensor([31, 30, 28, 24, 19, 13, 7, 1], device='cuda')
    elif patch_option == 3:
        annulino = torch.tensor([0, 12, 24, 36, 48, 60, 72, 84, 90], device='cuda')
        skyvaultaltint = torch.tensor([6, 18, 30, 42, 54, 66, 78, 90], device='cuda')
        azistart = torch.tensor([0, 4, 2, 5, 8, 0, 10, 0], device='cuda')
        patches_in_band = torch.tensor([31*2, 30*2, 28*2, 24*2, 19*2, 13*2, 7*2, 1], device='cuda')
    elif patch_option == 4:
        annulino = torch.tensor([0, 4.5, 9, 15, 21, 27, 33, 39, 45, 51, 57, 63, 69, 75, 81, 90], device='cuda')
        skyvaultaltint = torch.tensor([3, 9, 15, 21, 27, 33, 39, 45, 51, 57, 63, 69, 75, 81, 90], device='cuda')
        patches_in_band = torch.tensor([31*2, 31*2, 30*2, 30*2, 28*2, 28*2, 24*2, 24*2, 19*2, 19*2, 13*2, 13*2, 7*2, 7*2, 1], device='cuda')
        azistart = torch.tensor([0, 0, 4, 4, 2, 2, 5, 5, 8, 8, 0, 0, 10, 10, 0], device='cuda')

    skyvaultaziint = 360 / patches_in_band

    for j in range(skyvaultaltint.shape[0]):
        for k in range(patches_in_band[j]):
            skyvaultalt = torch.cat((skyvaultalt, torch.tensor([skyvaultaltint[j]], device='cuda')))
            skyvaultazi = torch.cat((skyvaultazi, torch.tensor([k * skyvaultaziint[j] + azistart[j]], device='cuda')))
